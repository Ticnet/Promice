...
- `task_score`: Final normalized score `[0.15, 0.85]` (emitted on `done=True`).
...
|---|---|
| **Tier 1 (Easy)** | `0.8360` |
| **Tier 2 (Medium)** | `0.8325` |
| **Tier 3 (Hard)** | `0.8290` |
| **Average Score** | **`0.8325`** |

*All scores are normalized to the OpenEnv standard range of `[0.15, 0.85]`.*
...
- **Intermittent Failures**: Correct actions may transiently fail (simulates network blips).
...
Enabled by setting `procedural=True` in `reset()`. The engine randomizes package names, versions, and paths while preserving the semantic markers of the error, preventing agents from overfitting to static strings.
